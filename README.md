...
> once** (llama-swap groups load several together) and **tens of GB of free disk**
...
> **You provide the llama-swap config yourself — this repo does not ship one.**
> llama-swap is driven by a YAML file mapping each model **alias** (the name the
> apps request) to the `llama-server` command that serves it. The aliases must
> equal the names in `.env` / `pipeline/settings.json` (see
> [How the model names link up](#how-the-model-names-link-up)). Rough shape:
>
> ```yaml
> models:
>   "Support_Agent_Qwen3.6":    # = backend LANGUAGE_MODEL
>     cmd: "llama-server -m /models/qwen3-vl.gguf --ctx-size 100000 --temp 0 --port ${PORT}"
>   "Embedding_Qwen3.6":        # = EMBEDDING_MODEL
>     cmd: "llama-server -m /models/qwen3-embed.gguf --embedding --port ${PORT}"
>   "Reranker_Qwen3.6":         # = RERANKER_MODEL
>     cmd: "llama-server -m /models/qwen3-rerank.gguf --reranking --ubatch-size 4096 --port ${PORT}"
> # plus the pipeline aliases (RAG_Pipeline_Pass34, RAG_Pipeline_Pass5Ingest) and
> # a `groups:` block so co-used models load together.
> ```
>
> Illustrative only — see the llama-swap docs
> (https://github.com/mostlygeek/llama-swap) for the exact schema and the flags
> for your version.
...
|---|---|
| Connection refused / errors calling the inference server (`:11400`) | llama-swap isn't running, or `INFERENCE_HOST` points at the wrong host/port. Start the server and check `curl http://localhost:11400/v1/models`. Point `INFERENCE_HOST` at the llama-swap **proxy** port, not an upstream model port. |
...
| Agent loops (many `db_search_*` calls) then returns a garbled non-answer | Every retrieval is erroring (often the DB-auth issue above) or returning nothing, so the model retries until `MAX_AGENT_ITERS`. Check the `trace` in the raw `/chat` JSON and the backend console. |
