...
> 模型**（llama-swap 群組會一次載入多個），以及 **數十 GB 的可用磁碟空間**存放
...
> **llama-swap 設定需由您自行提供——本專案並未內附。** llama-swap 由一個 YAML
> 檔驅動，將每個模型**別名**（應用程式請求的名稱）對應到提供該模型的
> `llama-server` 命令。這些別名必須與 `.env` / `pipeline/settings.json` 中的名稱
> 一致（見[模型名稱如何串接](#模型名稱如何串接)）。大致形式如下：
>
> ```yaml
> models:
>   "Support_Agent_Qwen3.6":    # = 後端 LANGUAGE_MODEL
...
> # 另需管線別名（RAG_Pipeline_Pass34、RAG_Pipeline_Pass5Ingest），以及一個
> # `groups:` 區塊，使共用的模型一起載入。
> ```
>
> 僅供示意——確切的 schema 與適用於您版本的參數請見 llama-swap 文件
...
| 症狀 | 可能原因／解法 |
|---|---|
| 連線被拒／呼叫推論伺服器（`:11400`）時發生錯誤 | llama-swap 未啟動，或 `INFERENCE_HOST` 指向錯誤的主機／埠。請啟動伺服器並以 `curl http://localhost:11400/v1/models` 驗證。請將 `INFERENCE_HOST` 指向 llama-swap 的**代理**埠，而非上游模型埠。 |
...
| 代理不斷呼叫 `db_search_*` 後回傳無意義的答覆 | 每次檢索都出錯（常是上述資料庫認證問題）或無結果，模型便一直重試直到 `MAX_AGENT_ITERS`。請檢視 `/chat` 原始 JSON 的 `trace` 與後端主控台。 |
