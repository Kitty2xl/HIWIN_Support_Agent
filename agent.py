...
async def _completeness_pass(question: str, draft: str, passages: list) -> tuple[str, int, list]:
    ...
    messages = [{
        ...
        "content": (
            ...
            "COMPLETENESS: the draft may have OMITTED entire product types or series "
            "the extractions cover. Do NOT let the draft's scope limit the answer — "
            "include every extracted entry; do NOT drop, merge, or summarize any "
            "away, and do NOT invent entries absent from the extractions.\n"
            "VOICE: write as a warm, consultative HIWIN application engineer. Open "
            "with one brief, friendly sentence framing what you found, add light "
            "guidance where it helps the reader choose, and close by offering to "
            "help narrow things down. Polished and professional — no slang, no emoji.\n"
            "LAYOUT: organize by the attribute the user asked about (e.g. group by "
            "lead / 導程, or by type), NOT by source page — do not create per-page "
            "sections. When the same model/part number repeats, append the spec that "
            "distinguishes the rows (e.g. 珠徑 / ball diameter) so variants aren't "
            "mistaken for duplicates. Cite each entry inline with its [Page N] tag. "
            "Write in the user's language.\n\n"
            f"=== USER REQUEST ===\n{question}\n\n"
            f"=== DRAFT ANSWER (voice/format reference only) ===\n{draft}\n\n"
